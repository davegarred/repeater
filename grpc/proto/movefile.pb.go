// Code generated by protoc-gen-go. DO NOT EDIT.
// source: movefile.proto

/*
Package grpcfile is a generated protocol buffer package.

It is generated from these files:
	movefile.proto

It has these top-level messages:
	Filecontent
	Filekey
*/
package grpcfile

import proto "github.com/golang/protobuf/proto"
import fmt "fmt"
import math "math"

import (
	context "golang.org/x/net/context"
	grpc "google.golang.org/grpc"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion2 // please upgrade the proto package

type Filecontent struct {
	Content string `protobuf:"bytes,1,opt,name=content" json:"content,omitempty"`
}

func (m *Filecontent) Reset()                    { *m = Filecontent{} }
func (m *Filecontent) String() string            { return proto.CompactTextString(m) }
func (*Filecontent) ProtoMessage()               {}
func (*Filecontent) Descriptor() ([]byte, []int) { return fileDescriptor0, []int{0} }

func (m *Filecontent) GetContent() string {
	if m != nil {
		return m.Content
	}
	return ""
}

type Filekey struct {
	Key string `protobuf:"bytes,1,opt,name=key" json:"key,omitempty"`
}

func (m *Filekey) Reset()                    { *m = Filekey{} }
func (m *Filekey) String() string            { return proto.CompactTextString(m) }
func (*Filekey) ProtoMessage()               {}
func (*Filekey) Descriptor() ([]byte, []int) { return fileDescriptor0, []int{1} }

func (m *Filekey) GetKey() string {
	if m != nil {
		return m.Key
	}
	return ""
}

func init() {
	proto.RegisterType((*Filecontent)(nil), "grpcfile.Filecontent")
	proto.RegisterType((*Filekey)(nil), "grpcfile.Filekey")
}

// Reference imports to suppress errors if they are not otherwise used.
var _ context.Context
var _ grpc.ClientConn

// This is a compile-time assertion to ensure that this generated file
// is compatible with the grpc package it is being compiled against.
const _ = grpc.SupportPackageIsVersion4

// Client API for Filemover service

type FilemoverClient interface {
	Pushfile(ctx context.Context, in *Filecontent, opts ...grpc.CallOption) (*Filekey, error)
}

type filemoverClient struct {
	cc *grpc.ClientConn
}

func NewFilemoverClient(cc *grpc.ClientConn) FilemoverClient {
	return &filemoverClient{cc}
}

func (c *filemoverClient) Pushfile(ctx context.Context, in *Filecontent, opts ...grpc.CallOption) (*Filekey, error) {
	out := new(Filekey)
	err := grpc.Invoke(ctx, "/grpcfile.Filemover/Pushfile", in, out, c.cc, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// Server API for Filemover service

type FilemoverServer interface {
	Pushfile(context.Context, *Filecontent) (*Filekey, error)
}

func RegisterFilemoverServer(s *grpc.Server, srv FilemoverServer) {
	s.RegisterService(&_Filemover_serviceDesc, srv)
}

func _Filemover_Pushfile_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(Filecontent)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(FilemoverServer).Pushfile(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: "/grpcfile.Filemover/Pushfile",
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(FilemoverServer).Pushfile(ctx, req.(*Filecontent))
	}
	return interceptor(ctx, in, info, handler)
}

var _Filemover_serviceDesc = grpc.ServiceDesc{
	ServiceName: "grpcfile.Filemover",
	HandlerType: (*FilemoverServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "Pushfile",
			Handler:    _Filemover_Pushfile_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "movefile.proto",
}

func init() { proto.RegisterFile("movefile.proto", fileDescriptor0) }

var fileDescriptor0 = []byte{
	// 139 bytes of a gzipped FileDescriptorProto
	,
}
